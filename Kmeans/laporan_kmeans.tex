...
\section{Hasil Clustering dengan Orange Data Mining}
Selain komputasi manual dan \textit{library scikit-learn}, proses \textit{clustering} K-Means juga diimplementasikan dan divisualisasikan menggunakan \textit{Orange Data Mining}. Berikut adalah hasil \textit{scatter plot} dan tabel data yang diperoleh:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/orange_scratch_plot.png}
    \caption{Plot Visualisasi Hasil Clustering dengan Orange Data Mining}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{image/orange_table.jpg}
    \caption{Tabel Data Hasil Clustering dengan Orange Data Mining}
\end{figure}

\section{Lampiran Kode Program}

\subsection{Program K-Means *from Scratch* (Perhitungan Manual)}
...
\lstinputlisting[caption={Kode sumber kmeans\_scratch.py}]{kmeans_scratch.py}

\subsection{Program K-Means Menggunakan Library}
Program ini menggunakan \textit{scikit-learn} untuk pencarian jumlah cluster terbaik (\textit{Elbow Method}) dan proses *clustering*.

\lstinputlisting[caption={Kode sumber kmeans\_library.py}]{kmeans_library.py}
